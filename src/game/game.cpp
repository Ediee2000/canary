...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isEvent /*= false*/)
{
	...
	if (damage.primary.value > 0) {
		...
		if (attackerPlayer)
		{
			...
			attackerPlayer->getStorageValue(8512, multiplyDmg);
			attackerPlayer->getStorageValue(8520, increaseAllDmg);
			attackerPlayer->getStorageValue(8521, increasePhyDmg);
			attackerPlayer->getStorageValue(8522, increaseMagicDmg);
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		if (targetPlayer && attacker && attacker->getMonster()) {
			//Charm rune (target as player)
			MonsterType* mType = g_monsters().getMonsterType(attacker->getMonsterTypeName());
			if (mType) {
